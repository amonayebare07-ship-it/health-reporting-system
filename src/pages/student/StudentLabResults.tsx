...
export default function StudentLabResults() {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from('lab_results').select('*').eq('student_id', user.id).order('created_at', { ascending: false })
          ...
          <CardContent className="p-0">
            <Table>
              ...
              <TableBody>
                ...
                {results.length === 0 && (
                  <TableRow><TableCell colSpan={5} className="text-center text-muted-foreground py-8">No lab results</TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          </CardContent>
